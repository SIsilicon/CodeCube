...
func _on_Delete_pressed():
	...
	undo_redo.add_undo_property(program, "links", program.links.duplicate())
	for block in selected_blocks:
		...
		undo_redo.add_do_method(get_parent(), "remove_block", block)
...
func _on_Copy_pressed():
	...
	for block in selected_blocks:
		...
		undo_redo.add_undo_method(get_parent(), "remove_block", new_block)
	...
	for old_block in new_blocks:
		...
		for i in sockets.size():
			if sockets[i].type == 1:
				var links : Array = program.get_socket_links(sockets[i])
				if links.size() > 0:
					var other_socket : Panel = program.get_socket(links[0], true)
					...
					if other_block in new_blocks.keys():
						...
						undo_redo.add_do_method(program, "add_link", new_socket_out.id, new_socket_in.id, program.link_key)
